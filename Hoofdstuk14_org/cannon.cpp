...
void CannonField::moveShot()
{
    ...
    if ( shotR.intersects( targetRect() ) ) {
        ...
        emit hit();
        emit canShoot( TRUE );
    } else if (shotR.intersects(barrierRect())) {
        fall = 135;
...
void CannonField::paintWind( QPainter *p )
{
        QString s = "Windrichting: >>  Windkracht: " + QString::number(wind);
        p->setPen( black );
        p->setFont( QFont( "Courier", 12, QFont::Bold ) );
        p->drawText( rect(), AlignTop, s );
}
...
QRect CannonField::shotRect() const
{
    ...
    double x         = x0 + velx*time + wind*time*time;
